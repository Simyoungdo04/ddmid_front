...
function RouteMap({ routes, startPoint, endPoint, startLabel = '출발', endLabel = '도착', height = 320 }) {
  ...
  function styleFor(route) {
    return {
      strokeWeight: route.highlight ? 7 : 4,
      strokeOpacity: route.dimmed ? 0.35 : 0.85,
    }
  }

  useEffect(() => {
    if (!KAKAO_JS_KEY || (validRoutes.length === 0 && !startPoint)) return
    ...
    script.onload = () => {
      window.kakao.maps.load(() => {
        ...
        if (startPoint) {
          ...
          addLabeledMarker(map, startLatLng, startLabel, '#2563eb')
          bounds.extend(startLatLng)
        }
        if (endPoint) {
          const endLatLng = new window.kakao.maps.LatLng(endPoint.lat, endPoint.lng)
          addLabeledMarker(map, endLatLng, endLabel, '#059669')
  ...
  }, [JSON.stringify(validRoutes.map((r) => ({ points: r.points, color: r.color, dash: r.dash, detail: r.detail }))), JSON.stringify(startPoint), JSON.stringify(endPoint), startLabel, endLabel])
  ...
  function showDetail(map, position, text) {
    ...
    detailOverlayRef.current = new window.kakao.maps.CustomOverlay({
      ...
      map,
      yAnchor: 1.4,
      content: `<div style="background:#111827;color:#fff;padding:6px 10px;border-radius:8px;font-size:12px;max-width:220px;box-shadow:0 2px 8px rgba(0,0,0,.3);">${escapeHtml(text).replace(/\n/g, '<br/>')}</div>`,
    })
  }
